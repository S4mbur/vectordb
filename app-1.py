...
ES_BULK_CHUNK_SIZE = env_int("ES_BULK_CHUNK_SIZE", 16)
ES_BULK_MAX_CHUNK_BYTES = env_int("ES_BULK_MAX_CHUNK_BYTES", 5 * 1024 * 1024)
ES_BULK_MAX_RETRIES = env_int("ES_BULK_MAX_RETRIES", 5)
ES_BULK_INITIAL_BACKOFF = env_float("ES_BULK_INITIAL_BACKOFF", 1.0)
ES_BULK_MAX_BACKOFF = env_float("ES_BULK_MAX_BACKOFF", 30.0)
ES_REFRESH_AFTER_UPLOAD = env_bool("ES_REFRESH_AFTER_UPLOAD", True)
ES_STOP_ON_BATCH_ERROR = env_bool("ES_STOP_ON_BATCH_ERROR", True)
ES_ERROR_SAMPLE_LIMIT = env_int("ES_ERROR_SAMPLE_LIMIT", 5)
...
PDF_EXTRACTOR = os.getenv("PDF_EXTRACTOR", "pymupdf").strip().lower()
PDF_TEXT_SORT = env_bool("PDF_TEXT_SORT", True)
PDF_FALLBACK_TO_PYPDF = env_bool("PDF_FALLBACK_TO_PYPDF", True)
...
def extract_pdf_pages_with_pymupdf(file_path: str) -> List[Tuple[int, str]]:
    if pymupdf is None:
    ...
    with pymupdf.open(file_path) as document:
        for page_index, page in enumerate(document):
            text = page.get_text("text", sort=PDF_TEXT_SORT) or ""
            if text.strip():
                pages.append((page_index + 1, text))
    return pages


def extract_pdf_pages_with_pypdf(file_path: str) -> List[Tuple[int, str]]:
...
def extract_pdf_pages(file_path: str) -> List[Tuple[int, str]]:
    ...
    if extractor in {"pymupdf", "fitz"}:
        try:
            return extract_pdf_pages_with_pymupdf(file_path)
        except Exception as exc:
            if not PDF_FALLBACK_TO_PYPDF:
                raise
            logger.warning(
                "PyMuPDF extraction failed; falling back to pypdf. file=%s error=%s",
                file_path,
                exc,
            )
            return extract_pdf_pages_with_pypdf(file_path)

    if extractor == "pypdf":
        return extract_pdf_pages_with_pypdf(file_path)

    raise RuntimeError(
        f"Unknown PDF_EXTRACTOR={PDF_EXTRACTOR!r}. Use pymupdf or pypdf."
    )


def extract_pdf_text(file_path: str) -> str:
    """Compatibility wrapper. New upload code uses extract_and_chunk_pdf()."""
    return "\n".join(
        f"\n--- PAGE {page_no} ---\n{text}"
        for page_no, text in extract_pdf_pages(file_path)
    )
...
def split_sentences(text: str) -> List[str]:
    ...
    if not normalized:
        return []
    values = [x.strip() for x in SENTENCE_SPLIT_RE.split(normalized) if x.strip()]
...
def chunk_pdf_pages(pages: List[Tuple[int, str]]) -> List[str]:
    raw_chunks: List[str] = []
    for page_no, page_text in pages:
        paragraphs = [
            normalize_text_space(value)
            for value in PARAGRAPH_SPLIT_RE.split(page_text or "")
            if value.strip()
        ]
        units: List[str] = []
    ...
    return [chunk for chunk in raw_chunks if count_words(chunk) >= CHUNK_MIN_WORDS]


def chunk_text(text: str) -> List[str]:
    pages: List[Tuple[int, str]] = []
    for page_no, page_text in split_text_pages(text):
        numeric_page = int(page_no) if page_no and str(page_no).isdigit() else len(pages) + 1
        pages.append((numeric_page, page_text))
    return chunk_pdf_pages(pages)


def extract_and_chunk_pdf(file_path: str) -> Dict[str, Any]:
    extract_started = now()
    pages = extract_pdf_pages(file_path)
    extract_time = elapsed(extract_started)

    chunk_started = now()
    chunks = chunk_pdf_pages(pages)
    chunk_time = elapsed(chunk_started)

    total_words = sum(count_words(page_text) for _, page_text in pages)
    logger.info(
        "PDF processed file=%s extractor=%s pages=%s words=%s chunks=%s "
        "extract=%ss chunk=%ss total=%ss",
        ...
        extract_time,
        chunk_time,
        round(extract_time + chunk_time, 4),
    )
...
def elastic_error_reason(error: Any) -> str:
    ...
    if caused_by:
        parts.append("caused_by=" + elastic_error_reason(caused_by))
    return ": ".join(parts) if parts else json.dumps(error, ensure_ascii=False)[:1000]


def elastic_bulk_failure_detail(item: Dict[str, Any]) -> Dict[str, Any]:
    if not item:
        return {"status": None, "id": None, "reason": "Empty bulk response item"}
    operation, detail = next(iter(item.items()))
    detail = detail or {}
    return {
        "operation": operation,
        "status": detail.get("status"),
        "id": detail.get("_id"),
        "index": detail.get("_index"),
        "reason": elastic_error_reason(detail.get("error")),
    }


def index_elasticsearch_actions(
    ...
    try:
        for ok, item in helpers.streaming_bulk(
            client,
            actions,
            chunk_size=max(1, ES_BULK_CHUNK_SIZE),
            max_chunk_bytes=max(1024 * 1024, ES_BULK_MAX_CHUNK_BYTES),
            max_retries=max(0, ES_BULK_MAX_RETRIES),
            initial_backoff=max(0.1, ES_BULK_INITIAL_BACKOFF),
            max_backoff=max(1.0, ES_BULK_MAX_BACKOFF),
            ...
            failed += 1
            if len(samples) < max(1, ES_ERROR_SAMPLE_LIMIT):
                samples.append(elastic_bulk_failure_detail(item))
    except Exception as exc:
        raise RuntimeError(
            f"Elasticsearch bulk transport failure for {len(actions)} documents: {exc}"
        ) from exc

    return succeeded, failed, samples


def format_elasticsearch_bulk_error(
    file_name: str,
    succeeded: int,
    failed: int,
    samples: List[Dict[str, Any]],
    ...
    sample_text = "; ".join(
        (
            f"status={sample.get('status')} id={sample.get('id')} "
            f"reason={sample.get('reason')}"
        )
        for sample in samples
    )
    return (
        f"Elasticsearch failed to index {failed} documents for {file_name}; "
        f"successful={succeeded}. Samples: {sample_text or 'no error details returned'}"
    )
...
def pg_process_upload_files(job_id: str, saved_files: List[Dict[str, str]]) -> Dict[str, Any]:
    total_start = now()
    extract_time = chunk_time = embed_time = db_time = 0.0
    inserted_total = 0
    results = []
    ...
    try:
        with conn:
            with conn.cursor() as cur:
                for saved in saved_files:
                    ...
                    pdf_result = extract_and_chunk_pdf(file_path)
                    chunks = pdf_result["chunks"]
                    extract_time += pdf_result["extract_time"]
                    chunk_time += pdf_result["chunk_time"]
        ...
        return {
            ...
            "files": results,
            "timings": {
                "pdf_extract": round(extract_time, 4),
                "chunk_build": round(chunk_time, 4),
                "extract_chunk": round(extract_time + chunk_time, 4),
...
def elasticsearch_process_upload_files(job_id: str, saved_files: List[Dict[str, str]]) -> Dict[str, Any]:
    ...
    for saved in saved_files:
        ...
        pdf_result = extract_and_chunk_pdf(file_path)
        chunks: List[str] = pdf_result["chunks"]
        extract_time += pdf_result["extract_time"]
        chunk_time += pdf_result["chunk_time"]
        increment_upload_job(job_id, chunks_total=len(chunks))
        ...
        def flush_pending_actions() -> None:
            nonlocal db_time, file_succeeded, file_failed, failure_samples, pending_actions
            ...
            update_upload_job(
                job_id,
                message=(
                    f"Indexing {file_name} into Elasticsearch: "
                    f"{file_succeeded}/{len(chunks)} completed."
                ),
            )
            started = now()
            succeeded, failed, samples = index_elasticsearch_actions(client, pending_actions)
            db_time += elapsed(started)
            file_succeeded += succeeded
            file_failed += failed
            failure_samples.extend(samples[: max(0, ES_ERROR_SAMPLE_LIMIT - len(failure_samples))])

            if succeeded:
                increment_upload_job(job_id, inserted_chunks=succeeded)

            pending_actions = []

            if failed and ES_STOP_ON_BATCH_ERROR:
                raise RuntimeError(
                    format_elasticsearch_bulk_error(
                        file_name,
                        file_succeeded,
                        file_failed,
                        failure_samples,
        ...
        for chunk_start in range(0, len(chunks), embedding_batch_size):
            chunk_batch = chunks[chunk_start : chunk_start + embedding_batch_size]

            update_upload_job(
                job_id,
                message=(
                    f"Embedding {file_name}: "
                    f"{chunk_start}/{len(chunks)} chunks."
                ),
            )
            started = now()
            inputs: Any = chunk_batch[0] if len(chunk_batch) == 1 else chunk_batch
            embeddings = request_embeddings(inputs)
            embed_time += elapsed(started)
            increment_upload_job(job_id, chunks_done=len(chunk_batch))

            for relative_index, (chunk, embedding) in enumerate(zip(chunk_batch, embeddings)):
                chunk_index = chunk_start + relative_index
                ...
                if len(pending_actions) >= max(1, ES_BULK_CHUNK_SIZE):
        ...
        if file_succeeded and ES_REFRESH_AFTER_UPLOAD:
            started = now()
            client.indices.refresh(index=ES_INDEX)
            db_time += elapsed(started)

        inserted_total += file_succeeded
        increment_upload_job(
            job_id,
            files_done=1,
            message=f"Finished {file_name} in Elasticsearch.",
        )
        record_upload_file_result(job_id, file_name, "ok", file_succeeded)
        results.append(
            {
                "file": file_name,
                "status": "ok",
                "chunks": file_succeeded,
            }
        )
...
async function loadAppConfig() {
  try {
    ...
    console.error("App config could not be loaded:", error);
  }
...
function toggleMilvusTools() {
  if (!milvusDiagnosticsEnabled || backend !== "milvus") {
    return;
  }
...
function setBackend(value) {
  ...
  ["pg", "milvus", "oracle", "qdrant", "elastic"].forEach((name) => {
    const element = document.getElementById(name + "Tab");
    if (element) {
      element.classList.remove("active");
    }
  });

  const tabId = value === "elasticsearch" ? "elasticTab" : value + "Tab";
  const activeTab = document.getElementById(tabId);
  if (activeTab) {
    activeTab.classList.add("active");
  }

  document.getElementById("backendText").innerText =
    "Active backend: " + (backendNames[value] || value);
...
function baseUrl() {
  return backend === "elasticsearch" ? "/elasticsearch" : "/" + backend;
}
...
function formatAnswer(text) {
  const safeText = escapeHtml(text || "No answer.");
...
function renderBot(element, data) {
  element.className = "msg bot formatted";
  ...
  if (Array.isArray(data.sources) && data.sources.length > 0) {
    html += `<div class="section"><div class="label">Sources</div>`;
    ...
    data.sources.forEach((source, index) => {
      ...
      const rerankText = Number.isFinite(rerankScore)
      ...
      html += `
        <div class="source">
          ...
          <div>
            ...
            <div class="muted">chunk ${escapeHtml(source.chunk_index)}</div>
          </div>
          <div class="muted">sim ${similarityText}${rerankText}</div>
        </div>
      `;
    });
  ...
  if (data.timings) {
    html += `<div class="section"><div class="label">Timings</div><div class="chips">`;

    Object.entries(data.timings).forEach(([key, value]) => {
      html += `<span class="chip">${escapeHtml(key)}: ${escapeHtml(value)}s</span>`;
    });
...
function uploadJobText(job) {
  const lines = [
    ...
    `Chunks: ${job.chunks_done || 0}/${job.chunks_total || 0}`,
    `Inserted: ${job.inserted_chunks || 0}`
  ];
  ...
  if (job.status === "success") {
    const timings = timingText(job.timings);
    if (timings) {
      lines.push(timings.trim());
    }
  }
...
async function pollJob(jobId, jobBackend) {
  ...
  while (activeUploadJobId === jobId) {
    try {
      ...
      if (data.status === "success" || data.status === "failed") {
        ...
        if (data.status === "success" && backend === jobBackend) {
          await listDocs();
        }
      ...
      status.innerText = "Job poll error: " + error;
      activeUploadJobId = null;
      return;
    }
...
async function uploadPdfs() {
  ...
  status.innerText = "Queueing upload...";
  ...
  try {
    ...
    await pollJob(data.job_id, uploadBackend);
  } catch (error) {
    status.innerText = "Request error: " + error;
  }
}
...
async function listDocs() {
  ...
  try {
    ...
    if (!Array.isArray(data.documents) || data.documents.length === 0) {
      box.innerHTML = "<p>No documents.</p>";
      return;
    }
    ...
    data.documents.forEach((documentItem) => {
      html += `<tr><td>${escapeHtml(documentItem.file_name)}</td><td>${escapeHtml(documentItem.chunks)}</td></tr>`;
    });
...
async function clearDocs() {
  ...
  if (activeUploadJobId) {
    status.innerText = "Wait for the upload job to finish.";
    return;
  }
  ...
  try {
    const response = await fetch(baseUrl() + "/clear", { method: "DELETE" });
    const data = await response.json();
...
async function askQuestion() {
  ...
  input.value = "";
  const loadingMessage = addMessage("Searching documents...", "bot");
  ...
  try {
    ...
    if (!response.ok) {
      loadingMessage.innerText = "Error: " + JSON.stringify(data);
      return;
    }

    renderBot(loadingMessage, data);
  } catch (error) {
    loadingMessage.innerText = "Request error: " + error;
  }
}
...
async function milvusDiagnosticAction(action) {
  ...
  try {
    const response = await fetch(`/milvus/diagnostics/${action}`, {
      method: "POST"
    });
    const data = await response.json();
    output.innerText = JSON.stringify(data, null, 2);
...
def health():
    return {
        ...
        "elasticsearch": {
            "index": ES_INDEX,
            "url": ES_URL,
            "bulk_chunk_size": ES_BULK_CHUNK_SIZE,
            "bulk_max_chunk_bytes": ES_BULK_MAX_CHUNK_BYTES,
            "bulk_max_retries": ES_BULK_MAX_RETRIES,
        },
        "pdf": {
            "extractor": PDF_EXTRACTOR,
            "text_sort": PDF_TEXT_SORT,
            "fallback_to_pypdf": PDF_FALLBACK_TO_PYPDF,
        },
    }
