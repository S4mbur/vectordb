...
MILVUS_INDEX_TYPE = os.getenv("MILVUS_INDEX_TYPE", "HNSW").upper()
...
def get_milvus_collection() -> Collection:
    ...
    if not utility.has_collection(MILVUS_COLLECTION):
        ...
        collection_kwargs = {
            ...
            "schema": CollectionSchema(fields, description="PDF RAG collection"),
            "shards_num": 1,
        }
        try:
            ...
            # Compatibility with older pymilvus versions.
        ...
        logger.info(
            "Created Milvus collection=%s index=%s consistency=%s",
            MILVUS_COLLECTION,
            MILVUS_INDEX_TYPE,
...
def milvus_process_upload_files(job_id: str, saved_files: List[Dict[str, str]]) -> Dict[str, Any]:
    ...
    for saved in saved_files:
        ...
        if MILVUS_FLUSH_AFTER_UPLOAD and rows:
            milvus_progress(
                job_id,
            ...
            logger.info(
                "Milvus manual flush skipped file=%s consistency=%s",
                file_name,
                MILVUS_CONSISTENCY_LEVEL,
            )
...
def milvus_diagnostics_action(action: str):
    global MILVUS_LOADED
    require_milvus_diagnostics()
    allowed = {"reconnect", "load", "release", "flush"}
